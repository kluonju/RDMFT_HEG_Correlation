#!/usr/bin/env python3
"""Optimize optGeo mixing angles (alpha, beta, gamma) on the unit sphere.

The C++ functional ``OptGeo@a;b;c`` (semicolons: commas are reserved in ``--funcs``)
...
def run_rdmft(
    ...
    key = f"OptGeo@{alpha:.12g};{beta:.12g};{gamma:.12g}"
    cmd = [
        str(exe),
        "--funcs",
        key,
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Optimize optGeo angles vs PW92 E_c (loose mode by default).",
    ...
    log("")
    log("=== optGeo angle search (RMSE of E_c vs PW92) ===")
    ...
    log(
        ...
        f"OptGeo@{a_r:.{args.decimals}f};{b_r:.{args.decimals}f};{c_r:.{args.decimals}f}"
    )
